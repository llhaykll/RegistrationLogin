...
import java.util.*;
/**
 * The Mail_ru class extends the Email class and represents a simplified Mail.ru-like email system.
 ...
 *
 * Mail.ru users are stored in a HashMap called "mailUsers" using their email addresses as keys.
 * User data is serialized and saved to files on the local file system.
 */

public class Mail_ru extends Email {
    ...
    private static Map<String,User> mailUsers = new HashMap<>();
     ...
     * After successful registration, the user data is added to the "mailUsers" map and saved to a file.
     */
    ...
    public void registration() {
        ...
        mailUsers.put(user.getUserEmailAddress(),user);
     ...
     */
    private User readUserData(String email) {
    ...
    private static String correctEmailAddressForReg(Scanner in) {
        ...
        while (true) {
            ...
            if (mailUsers.containsKey(email) || userFile.exists()) {
        ...
        String password;
                ...
                while (true) {
                    ...
                    if (password.length() >= 8) {
                        System.out.println(ANSI_YELLOW+"Registration successful!"+ANSI_RESET);
                        return password;
